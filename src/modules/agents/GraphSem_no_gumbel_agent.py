...
from modules.layers.multi_head_soft_attention import MultiHeadSoftAttention
from modules.layers.transformer_decoder import TransformerDecoder


class GraphSem_no_gumbel_agent(nn.Module):
    def __init__(self, input_shape, args):
        super(GraphSem_no_gumbel_agent, self).__init__()
        self.args = args
        ...
        self.soft_attention = MultiHeadSoftAttention(self.att_input_shape, self.att_input_shape,
    ...
    def forward(self, inputs, hidden_state):
        ...
        # soft-attention (no Gumbel-Softmax)
        soft_weights = self.soft_attention(comm, comm, mask)
        soft_weights = soft_weights.reshape(-1, self.args.enc_att_heads, self.args.n_agents, self.args.n_agents)
        soft_weights = soft_weights.mean(dim=1)  # Average across heads instead of max
        tot_weights = soft_weights + torch.eye(self.args.n_agents, device=comm.device).repeat(h.shape[0], 1, 1)
