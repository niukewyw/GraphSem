...
# @File    : multi_head_soft_attention.py
# @Software: PyCharm
...
def mask_softmax(x, mask):
    if mask is None:
        ...
        return F.softmax(x.reshape(shape), dim=-1)[:, :, :, 0]
...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, queries, keys, mask):
        ...
        weights = mask_softmax(scores, mask)
...
class MultiHeadSoftAttention(nn.Module):
    def __init__(self, key_size, query_size, num_heads, embed_dim):
        super(MultiHeadSoftAttention, self).__init__()
    ...
    def forward(self, queries, keys, mask):
        ...
        soft_attention_weights = self.attention(queries, keys, mask)

        return soft_attention_weights 